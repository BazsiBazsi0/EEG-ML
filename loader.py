...
class FileLoader:
    ...
    def load_saved_files():
        ...
        for s in subjects:
            ...
            xs.append(
                np.load(
                    "generator/all_electrodes_103_patients_ch_level_1/x_sub_{}.npy".format(
                        s
                    )
                )
            )
            ys.append(
                np.load(
                    "generator/all_electrodes_103_patients_ch_level_1/y_sub_{}.npy".format(
                        s
                    )
                )
            )
        ...
        x_pre, y_pre, x_no_smote_pre, y_no_smote_pre = FileLoader.preprocessor(
            x_concat, y_concat
        )
        ...
        x = x_pre.reshape(
            patients, num_patients, x_pre.shape[1], x_pre.shape[2]
        ).astype(np.float32)
